...
public class LaSemanticoUtils {
    ...
    public static TabelaDeSimbolos.TipoLaVariavel verificarTipo(TabelaDeSimbolos tabela, LaSintaticoParser.IdentificadorContext ctx){
        ...
        if(!identificador.contains("[") && !identificador.contains("]")){ // Caso nao tenha dimensao.
            ...
            for(var xp: ctx.dimensao().exp_aritmetica()) 
                verificarTipo(tabela, xp); // So pode ser inteiro, mas nao precisa checar!
    ...
    public static TabelaDeSimbolos.TipoLaVariavel verificarTipo(TabelaDeSimbolos tabela, LaSintaticoParser.Parcela_unarioContext ctx) {
        ...
        if(ctx.IDENT() != null){ // Caso seja uma chamada de funcao, necessario checar parametros e setar o tipo de retorno.
            ...
            for(var exp: ctx.expressao()){
                TabelaDeSimbolos.TipoLaVariavel aux = verificarTipo(tabela, exp);
                if (ret == null) {
                    ret = aux;
                } else if (!verificarTipo(ret,aux)) {
                    ret = TabelaDeSimbolos.TipoLaVariavel.INVALIDO;
            ...
            if(tabela.existe(ctx.IDENT().getText())){
                ...
                for(var exp: ctx.expressao()){
                    tiposParametros.add(verificarTipo(tabela, exp)); // Adiciona todos os tipos dos parametros no ArrayList.
                }
